---
name: split-commit-gac
description: "Split oversized Git working-tree changes into multiple logical commits by grouping files by concern, proposing a commit plan, and executing sequential commits with a configurable command (default: gac). Use when users say one commit became too large, ask to split current changes before commit, or request automatic commit chunking during deep coding sessions."
---
...
For each group:
1. Stage only that group's hunks/files.
2. Run commit command from `references/command-customization.md`.
3. Check `git log -1 --oneline` and continue.

Default command template: `gac`

Important:
- If `gac` stages all files internally, it cannot preserve split boundaries.
- In that case, switch to staged-only commit template (see reference).
